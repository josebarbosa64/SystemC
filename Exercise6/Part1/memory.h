...
template <unsigned int n=1024>  //^Template to specify memory size default 1024
class memory: public sc_module, tlm_fw_transport_if<> { //^Create memory as target, thats why I inherit fw_transport  See Chap4 SS 32
    private:
    unsigned char mem[n];
    ...
    void b_transport(tlm_generic_payload &trans, sc_time &delay) {
        if(trans.get_address() >= 1024) {   //^Specified in exercise, if I get an address larger than 1024 then error 
