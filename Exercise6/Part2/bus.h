...
class Interconnect : public sc_module, public tlm_bw_transport_if<>, tlm_fw_transport_if<> {   //^Inherit from both Initiator and Target
    public:
    tlm_initiator_socket<> iSocket[2];  //^Two initiators and two target sockets
    ...
    void b_transport( tlm_generic_payload &trans, sc_time &delay) {
        if (trans.get_address() < 512) {   //^ Check address and send to corresponding port
